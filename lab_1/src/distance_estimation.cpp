...
void draw_line_distance(cv::Mat img, vector<vector<cv::Point2f>> marker_corners, cv::Mat camera_matrix, cv::Mat dist_coeffs, vector<cv::Vec3d> rvecs, vector<cv::Vec3d> tvecs){
    ...
    cv::Point2f end_point = (marker_corners[1][0] + marker_corners[1][1] + marker_corners[1][2] + marker_corners[1][3]) / 4.f;
    cv::line(img, start_point, end_point, cv::Scalar(255, 0, 0), 2);
...
int main(int argc, char* argv[]){
    ...
    while (key != 27 && web_cam.isOpened()){
        ...
        if(!marker_ids.empty() && marker_ids.size() == 2) {
            draw_line_distance(img, marker_corners, camera_matrix, dist_coeffs, rvecs, tvecs);
        }

        // Show image
        cv::imshow("Distance Estimation", img);
